...
local function callbackres(extra, success, result) -- Callback for res_user in line 27
  local user = 'user#id'..result.id
	...
	if is_banned(result.id, extra.chatid) then -- Ignore bans
            send_large_msg(chat, 'کاربر بن شده')
	elseif is_gbanned(result.id) then -- Ignore globall bans
	    send_large_msg(chat, 'کاربر گلوبالی بن شده')
	else    
...
function run(msg, matches)
  ...
  if not is_realm(msg) then
    if data[tostring(msg.to.id)]['settings']['lock_member'] == 'فعال' and not is_admin(msg) then
		  return 'گروه قفل ادد است.'
    end
  end
