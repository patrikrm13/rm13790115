...
function First( msg )
	...
	if ( not Xdata[tostring(msg.to.id)]["settings"]["کلمات ممنوع"] ) then
		Xdata[tostring(msg.to.id)]["settings"]["کلمات ممنوع"] = TMP
...
function InsertWord( data, word, msg )
	if ( not is_momod(msg) ) then
		send_large_msg ( get_receiver(msg) , "@" .. msg.from.username .. "فقط ادمین بات :|" );
		return
	end

	TTable = data[tostring(msg.to.id)]["settings"]["کلمات ممنوع"]
	if ( TTable ) then
		print("Grate the table is here i will add this word to it..")
		send_large_msg ( get_receiver(msg) , "The word " .. word .. " به لیست کلمات ممنوع اضافه شد :|" );
...
function RemoveWord( data, index, msg )
	if ( not is_momod(msg) ) then
		send_large_msg ( get_receiver(msg) , "@" .. msg.from.username .. "فقط ادمین" );
		return
	end
	...
	if ( TTable ) then
		...
		send_large_msg ( get_receiver(msg) , "The word " .. tostring(TTable[index]) .. " از لیست کلمات ممنوع پاک شد :|" );
...
function ClearWords( data, msg )
	TTable = data[tostring(msg.to.id)]["settings"]["کلمات ممنوع"]
	print( "تلاش در پاک کردن تمام کلمات ممنوع" )
	...
	if ( TTable ) then
		...
		print("نمی توانم از کلمات ممنوع پاکش کنم:|")
	end
end
...
function run( msg, matches )
	...
	elseif ( matches[1] == "list" ) then
		...
		for k,v in pairs( DData[tostring(msg.to.id)]["settings"]["کلمت مممنوع"] ) do
